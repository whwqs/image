...
class TextBarcodeRecognition(object):
    ...
    def pretreatment(self, img):
        ...
        """
        # 1.图片边缘有黑线或者其它杂色时使用
        # roi_text = self.clearBorder(roi_text)

        # 2.倾斜角度矫正
        roi_text = np.rot90(img)
        # 计算角度
        angle = self.getMinAreaRect(roi_text)[-1]
        # 校正文本
        rotated = self.rotateBound(roi_text, angle)

        # 3.形态学筛选出文字矩形区域，细化文字识别区域
    ...
    def process(self, image):
        ...
        # 4. 膨胀和腐蚀操作的核函数,可以调整(20,6)\(15,3)
        element1 = cv2.getStructuringElement(cv2.MORPH_RECT, (20, 6))
        element2 = cv2.getStructuringElement(cv2.MORPH_RECT, (15, 6))
